...
function ReviewColumn({
    ...
    return<div ref={colRef} className={cn("animate-marquee space-y-8 py-4",className)} style={{'--marquee-duration':duration}as React.CSSProperties}>{reviews.concat(reviews).map((imgSrc,reviewIndex)=>(
...
function Review({imgSrc,className,...props}:ReviewProps){
    ...
    return <div className={cn("animate-fade-in riunded-[2.25rem] bg-white p-6 opacity-0 shadow-xl shadow-slate-900/5",className)} style={{animationDelay}} {...props} >
        <Phone imgSrc={imgSrc}/>
    </div>
}
function ReviewGrid(){
    ...
    return<div ref={containRef} className="relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3">
        {isInView? (<>
        ...
        msPerPixel={10}
        />
        <ReviewColumn reviews={[...col2, ...col3[1]]} className=" hidden md:block" reviewClassName={(reviewIndex)=>reviewIndex>=col2.length?"lg:hidden":" "
        }
        msPerPixel={15}
        />
        <ReviewColumn reviews={col3.flat()} className=" hidden md:block" 
        msPerPixel={10}
        />
